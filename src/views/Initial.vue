    ...
    <div class="flex items-center justify-center min-h-screen bg-gray-100">
        ...
        <div class="p-8 rounded-lg shadow-lg w-full sm:w-full md:w-[60%] lg:w-[40%] relative bg-[#274461] bg-opacity-70">
            <div class="flex flex-col md:flex-row gap-3">
                <div class="mt-4 flex justify-center relative max-w-full md:max-w-52 max-h-52 rounded-full p-4 bg-[#274461]">
                    <img :src="avatarUrl" alt="Random Character" class="w-full h-full object-cover rounded-full border-[#0ed494] border-[5px] bg-[#274461]" />
                    <button @click="toggleMenu" class="absolute bottom-2 p-1 left-0 mb-2 mr-2 bg-[#274461] text-white rounded-full z-100">
                        <Icon icon="material-symbols:styler-outline" height="45" />
                    </button>
                    <button class="absolute bottom-2 right-0 mb-2 mr-2 bg-[#274461] p-1 text-white rounded-full z-100" @click="randomizeSeed">
                ...
                <div class="flex flex-col gap-3 justify-center w-full">
                    ...
                    <div class="w-full">
                        <input v-model="name" type="text" placeholder="Your Name" class="w-full p-2 border border-gray-300 rounded-md bg-gray-400 bg-opacity-50 font-bold text-white text-2xl" />
                    </div>
                    <div class="w-full cursor-pointer">
                        <select v-model="selectedOption" class="w-full p-2 border border-gray-300 rounded-md bg-gray-400 bg-opacity-50 font-bold text-2xl text-white cursor-pointer">
            ...
            <div class="mt-3">
                <button @click="handleJoin" class="bg-[#0ed494] text-[#274461] p-3 text-xl rounded-lg w-full font-bold transform transition-all duration-150 ease-in-out shadow-[4px_4px_10px_rgba(0,0,0,0.5)] active:scale-95 active:shadow-none">
                    JOIN
                </button>
            </div>
        </div>
        ...
        <div v-if="isMenuOpen" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-[#0ed494] bg-opacity-90 p-6 rounded-lg shadow-lg w-[80vw] sm:w-[95vw] md:w-[80vw] lg:w-[80vw]">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4">
                    <div v-for="(style, key) in AVATAR_STYLES" :key="key" class="flex flex-col items-center">
                        <!-- Avatar Sample Image -->
                        <div @click="selectStyle(style)" class="justify-center flex flex-col items-center cursor-pointer bg-[#274461] w-40 rounded-lg p-2">
                            <img :src="`https://api.dicebear.com/9.x/${style}/svg?seed=random`" alt="Sample Avatar" class="w-24 h-24 object-cover rounded-full mb-2" />
                            <!-- Style Name Button -->
