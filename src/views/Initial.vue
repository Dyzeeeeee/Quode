<template>
    <div class="flex items-center justify-center min-h-screen bg-gray-100">
        ...
        <div class="p-8 rounded-lg shadow-lg w-full sm:w-full md:w-[60%] lg:w-[40%] relative bg-[#274461] bg-opacity-70">
            <div class="flex justify-center mb-5">
                <img src="@/assets/images/QuodE.png" alt="App Logo" class="xl:h-32 h-24" />
            </div>
            <div class="flex flex-col md:flex-row gap-3">
                <div class="mt-4 flex justify-center relative xl:max-w-52 xl:max-h-52 max-h-40 max-w-40 self-center rounded-full p-4 bg-[#274461]">
                    <img :src="avatarUrl" alt="Random Character" class="w-full h-full object-cover rounded-full border-[#0ed494] border-[5px] bg-emerald-400" />
                    <button @click="toggleMenu" class="absolute bottom-2 left-0 mb-2 p-1 bg-[#274461] text-white rounded-full">
                        <Icon icon="material-symbols:styler-outline" class="xl:h-[45px] xl:w-[45px] h-[35px] w-[35px]" />
                    </button>
                    <button @click="randomizeSeed" class="absolute bottom-2 right-0 mb-2 p-1 bg-[#274461] text-white rounded-full">
                        <Icon icon="game-icons:perspective-dice-six-faces-random" class="xl:h-[45px] xl:w-[45px] h-[35px] w-[35px]" />
                    </button>
                </div>
                <div class="flex flex-col gap-3 justify-center w-full">
                    ...
                    <div>
                        <select v-model="selectedSection" class="w-full p-2 border border-gray-300 rounded-md bg-gray-400 bg-opacity-50 text-white text-xl font-bold">
                            <option disabled value="">Select Section</option>
                            <option v-for="(section, index) in sectionGrouped" :key="index" :value="section.section_id" class="text-gray-700">
                                {{ section.section_name }}
                            </option>
                        </select>
                    </div>
                    <div v-if="selectedSection">
                        <select v-model="selectedName" class="w-full p-2 border border-gray-300 rounded-md bg-gray-400 bg-opacity-50 text-white text-xl font-bold">
                            <option disabled value="">Your Name</option>
                            <option v-for="user in sectionGrouped.find(s => s.section_id === selectedSection)?.users" :key="user.id" :value="user.id" class="text-gray-700">
                                {{ user.name }}
                            </option>
                        </select>
                    </div>
            ...
            <div class="mt-3">
                <button @click="handleJoin" class="bg-[#0ed494] text-[#274461] p-3 text-xl rounded-lg w-full font-bold shadow-lg active:scale-95">
                    JOIN
                </button>
            </div>
        </div>
        <div v-if="isMenuOpen" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-[#0ed494] bg-opacity-90 p-6 rounded-lg shadow-lg w-[100vw] h-[100vh] sm:w-[95vw] md:w-[80vw] lg:w-[90vw]">
                <h3 class="text-3xl font-bold mb-4 text-white">Select Avatar Style</h3>
                <div class="grid grid-cols-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-10 gap-4 lg:gap-2 h-[80vh] overflow-y-auto">
                    <div v-for="(style, key) in AVATAR_STYLES" :key="key" class="flex flex-col items-center gap-2">
                        <div @click="selectStyle(style)" class="cursor-pointer bg-[#274461] xl:w-40 w-24 h-30 rounded-lg p-2">
                            <img :src="`https://api.dicebear.com/9.x/${style}/svg?seed=random`" alt="Sample Avatar" class="xl:w-24 xl:h-24 w-16 h-16 object-cover rounded-full mb-2" />
